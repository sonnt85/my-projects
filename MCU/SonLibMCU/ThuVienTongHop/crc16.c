...
*/
unsigned int code crctable[256] = {
...
void CRC16_Update( unsigned int  *crcvalue, unsigned char poda ) {
	(*crcvalue) = ( (*crcvalue) << 8 ) ^ crctable[ ( (*crcvalue) >> 8 ) ^ poda ];
}

void CRC16_UpdateChecksum( unsigned int  *crcvalue, unsigned char *poda, uchar length ) {
	unsigned int crc;
	unsigned char *buf = (unsigned char *) poda;
...
unsigned int CRC16_BlockChecksum( unsigned char  *poda, uchar length ) {
	...
	CRC16_UpdateChecksum( &crc, poda, length );
