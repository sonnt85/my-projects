/*
 CRC-8
*/
#include <crc8.h>
#define CRC8_INIT_VALUE		0x0000
#define CRC8_XOR_VALUE		0x0000

#ifdef CREATE_CRC_TABLE

static unsigned char crctable[256];

/*
 Generate a table for a byte-wise 8-bit CRC calculation on the polynomial:
 x^8 + x^2 + x^1 + x^0
*/

void make_crc_table( void ){
	int i, j;
	unsigned long poly, c;
	/* terms of polynomial defining this crc (except x^8): */
	static const unsigned char p[] = {0,1,2};

	/* make exclusive-or pattern from polynomial (0x07) */
	poly = 0L;
	for ( i = 0; i < sizeof( p ) / sizeof( unsigned char ); i++ ) {
		poly |= 1L << p[i];
	}

	for ( i = 0; i < 256; i++ ) {
		c = i;
		for ( j = 0; j < 8; j++ ) {
			c = ( c * 0x80 ) ? poly ^ ( c << 1 ) : ( c << 1 );
		}
		crctable[i] = (unsigned char) c;
	}
}

#else

/*
 Table of CRC-8's of all single-byte values (made by make_crc_table)
*/
unsigned char code crctable[256] = {
	
};

#endif

void CRC8_InitChecksum( unsigned char *crcvalue ){
	*crcvalue = CRC8_INIT_VALUE;
}

void CRC8_Update( unsigned char *crcvalue, const unsigned char poda ){
	*crcvalue = crctable[(*crcvalue) ^ poda];
}

void CRC8_UpdateChecksum( unsigned char *crcvalue, const void *poda, int length )reentrant {
	unsigned char crc;
	const unsigned char *buf = (const unsigned char *) poda;

	crc = *crcvalue;
	while( length-- ) {
		crc = crctable[crc ^ *buf++];
	}
	*crcvalue = crc;
}

void CRC8_FinishChecksum( unsigned char *crcvalue ) {
	*crcvalue ^= CRC8_XOR_VALUE;
}

unsigned char CRC8_BlockChecksum( const void *poda, int length ) {
	unsigned char crc;

	CRC8_InitChecksum( &crc );
	CRC8_UpdateChecksum( &crc, poda, length );
	CRC8_FinishChecksum( &crc );
	return crc;
}
